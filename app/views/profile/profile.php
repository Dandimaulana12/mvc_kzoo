<?php
// $id_user = $_SESSION['id_user'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" href="<?=BASEURL;?>/img/logo.png">
    <title>Sign Up | K-ZOO</title>
    <style>
        html{
            font-family: Arial, Helvetica, sans-serif;
        }
        .cover{
            background-image: url('<?=BASEURL;?>/img/Login-Sign-Up.jpg');
            background-repeat: no-repeat;
            position: fixed;
            width: 100%;
            height: 100%;
            background-size: 100%;
            right: 0.5px;
            bottom: 0.5px;
        }
        .kotak_login{
            padding: 10px 10px;
            width: 405px;
            height: 520px;
            left: 385px;
            bottom: 195px;
            margin: 300px auto;
            text-align: center;
            border-radius: 8px;
            position: relative;
            background: rgba(209, 234, 197, 0.5);
            color: rgba(75, 122, 71, 1);
        }
        .kotak_login h3{
            font-size: 30px;
            color: rgba(75, 122, 71, 1);
        }
        form input[type="text"]:focus{
            outline-color: none;
            color: rgba(75, 122, 71, 1);
            font-size: 20px;
        }

        .kotak_login h3{
            position: relative;
            bottom: 10px;
            text-align: center;
        }
        .inp_username{
            border:  none;
            position: relative;
            background: rgba(157, 193, 131, 0.7);
            border-radius: 4px;
            width: 260px;
            height: 35px;
            margin-left: 10px;
            color: rgba(75, 122, 71, 1);
            font-size: 20px;
        }
        .inp_email{
            border:  none;
            position: relative;
            background: rgba(157, 193, 131, 0.7);
            border-radius: 4px;
            width: 260px;
            height: 35px;
            margin-left: -8px;
            color: rgba(75, 122, 71, 1);
            font-size: 20px;
        }
        .inp_password{
            border:  none;
            position: relative;
            background: rgba(157, 193, 131, 0.7);
            border-radius: 4px;
            width: 260px;
            height: 35px;
            margin-left: 10px;
            color: rgba(75, 122, 71, 1);
            font-size: 20px;
        }
        .lab_username{
            font-size: 20px;
        }
        .lab_email{
            font-size: 20px;
            margin-right: 18px;
        }
        .submit_log{
            border:  none;
            position: relative;
            background: rgba(157, 193, 131, 0.7);
            border-radius: 15px;
            width: 162px;
            height: 40px;
            margin-left: 10px;
            color: rgba(75, 122, 71, 1);
            font-size: 20px;
        }
        .logout{
            padding:10px; 
            padding-right:40px; 
            padding-left:40px; 
            background: rgba(157, 193, 131, 0.7);
            border-radius: 15px;
            position:relative; 
            top:45px;
            left: 21px;
            text-decoration: none;
            color: rgba(75, 122, 71, 1);
            font-size: 20px;
        }
        .logout:hover {
            background: rgba(157, 193, 131, 1);
            color: white;
        }

        .kotak_submit:hover .submit_log{
            background: rgba(157, 193, 131, 1);
            cursor: pointer;
            color: white;
        }
        .kotak_submit{
            position: relative;
            width: 162px;
            height: 40px;
            left: 130px;
            bottom: -25px;
        }
        .signup{
            font-size: 20px;
            position: relative;
            bottom: 20px;
        }
        .signup a{
           text-decoration: none;
           text-align: center;
           color: rgba(75, 122, 71, 1);
        }
        .signup_kotak{
            position: relative;
            background: rgba(157, 193, 131, 0.7);
            border-radius: 15px;
            width: 150px;
            height: 23px;
            top: -10px;
            left: 125px;
            margin-left: 10px;
            color: rgba(75, 122, 71, 1);
            font-size: 20px;
            padding-top: 10px;
            padding-bottom: 10px;
        }
        .signup .signup_kotak:hover {
            background: rgba(157, 193, 131, 1);
            cursor: pointer;
            color: white;
        }
        .signup_kotak:hover a{
            color: white;
        }
        .kotak_box{
            position: relative;
            top: 10px;
            left: 11px;
            z-index: 3;
        }
        .kotak_pw{
            width: 20px;
            height: 20px;
            border: none;
        }
        .kotak_terima{
            width: 20px;
            height: 20px;
        }
        .default-text-box{
            text-align: left;
        }
        .terima_text{
            font-size: 16px;
            position: relative;
        }
        .pw_text{
            font-size: 16px;
            position: relative;
        }
        .kotak_negara{
            position: relative;
            right: 0px;
            font-size: 20px;
        }
        .negara_pilih{
            width: 264px;
            height: 35px;
            border: none;
            border-radius: 4px;
            background: rgba(157, 193, 131, 0.7);
            color: rgba(75, 122, 71, 1);
        }
        /* css untuk menubar */
          /*start menu*/
        .nav ul{
            list-style-type: none;
        }
        .nav{
            display: inline-block;
            text-align: center;
            position: absolute;
            width: 100%;
            top: 8.5%;
            left: 53%;
            transform: translate(-50%,-50%);
        }
        .nav li:hover{
            background:chocolate;
        }
        .nav > ul {
            list-style: none;
            padding: 0;
            margin: 0;
            display: inline-block;
        }
        .nav > ul > li{
            position: relative;
            cursor: pointer;
            float: left;
            border: 0;
            border-radius: 70px;
            width: 130px;
            height: 35px;
            margin-right: 10px;
            line-height: 35px;
            background:darkorange;
            
        }
        .nav > ul > li > a{
          color: white;
          text-decoration: none;
            display: inline-block;
            text-align: center;
            position: absolute;
            width: 100%;
            top: 50.5%;
            left: 53%;
            transform: translate(-50%,-50%);
        }
        .dropdown{
            position: absolute;
            top: 100%;
            left: 0%;
            width: 100%;
            padding: 0;

        }
        .dropdown li{
            background: yellow;
            border-radius: 70px;
            background-color:darkorange;
            width: 180px;
            height: 36px;
            margin-top: 15px;
            right: 25px;
            line-height: 34px;
            position: relative;
        }
        .dropdown li a{
            color: white;
            text-decoration: none;
        }
        .dropdown li:hover{
            background:chocolate;
        }
        .dropdown li{
            display: none;
        }
        .nav li:hover > .dropdown li{
            display: block;
        }
        .nav li:hover > .dropdown{
            perspective: 1000px;
        }
        .nav li:hover > .dropdown li{
            transform-origin: top left;
            opacity: 0;
        }
        .nav li:hover > .dropdown li:nth-child(1){
            animation: animate 300ms ease-in-out forwards;
            animation-delay: -45ms;
        }
        .nav li:hover > .dropdown li:nth-child(2){
            animation: animate 300ms ease-in-out forwards;
            animation-delay: 0ms;
        }
        .nav li:hover > .dropdown li:nth-child(3){
            animation: animate 300ms ease-in-out forwards;
            animation-delay: 45ms;
        }
        .nav li:hover > .dropdown li:nth-child(4){
            animation: animate 300ms ease-in-out forwards;
            animation-delay: 90ms;
        }
        .nav li:hover > .dropdown li:nth-child(5){
            animation: animate 300ms ease-in-out forwards;
            animation-delay: 135ms;
        }
        @keyframes animate{
            0%{
                opacity: 0;
                transform: rotateZ(90deg);
            }
            70%{
                transform: skewX(10deg);
            }
            100%{
                opacity: 1;
                transform: rotateZ(0deg);
            }
        }
        /*end menu*/
        .gambar_ig{
            position: absolute;
            left: 1050px;
            top: 45px;
        }
        .gambar_ig svg{
            padding-right: 10px;
        }
        .gambar_ig a{
            text-decoration: none;
        }

        svg:hover path{
            fill: black;
            
        }
        /* end css menubar */
    </style>
    <link rel="stylesheet" href="<?=BASEURL;?>/aos/dist/aos.css" />
</head>
<body>
    <div class="cover">
         <!-- start menubar -->
           <div class="nav">
            <ul>
                <li><a href="1_index.php">HOME</a></li>
                <li><a href="2_gallery.php">GALLERY</a></li>
                <li><a href="#">TICKETS</a>
                    <ul class="dropdown">
                        <li><a href="3_ticket.php">Pembelian Ticket</a></li>
                        <li><a href="#">Riwayat Ticket</a></li>
                    </ul>
                </li>
                <li><a href="about.php">ABOUT US</a></li>
                <li class="login" style="display:block;" id="login"><a href="4_login.php">LOGIN</a></li>  
                <li class="login" style="display:block;" id="profile"><a href="profile.php" >PROFILE</a></li>  
            </ul>
        </div>
        <div class="gambar_ig">
            <a href="https://www.instagram.com"><svg style="color: white" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-instagram" viewBox="0 0 16 16"> <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z" fill="white"></path> </svg>
            </a>
            <a href="https://www.twitter.com"><svg style="color: white" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-twitter" viewBox="0 0 16 16"> <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z" fill="white"></path> </svg>
            </a>
            <a href="https://www.facebook.com"><svg style="color: white" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-facebook" viewBox="0 0 16 16"> <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z" fill="white"></path> </svg>
            </a>
        </div>
        <!-- end menubar -->

    <div class="aos-item" data-aos="zoom-out-up">
       <div class="kotak_login">
            <div data-aos="fade-right">
                <h3>PROFILE</h3>
            </div>

        <form action="<?=BASEURL;?>/profile/update" method="POST">
           <input type="hidden" value="<?= $_SESSION['id_user'];?>" name="id_user">
            <div data-aos="fade-left">
                <div class="kotak_username">
                    <label for="email" class="lab_email">Full Name :</label>
                    <input type="text" name="full_name" class="inp_email" value="<?= $data['full_name'];?>">
                </div>
            </div>
            <br>
            <br>
            <div data-aos="flip-left">
                <div class="kotak_username">
                    <label for="username" class="lab_username">Username :</label>
                    <input type="text" name="username" class="inp_username" value="<?php echo $data['username'];?>">
                </div>
            </div>
            <br>
            <div data-aos="flip-left">
                <div class="kotak_negara">
                    <label for="negara" class="lab_negara">Nationality  :</label>
                    <select name="negara" id="negara" class="negara_pilih">
                        
                        <option value="<?php echo $data["national"]; ?>"><?php echo $data["national"];?></option>
                        <option value="<?php if($data["national"] == "indonesian"){ echo "tourist"; }else{ echo "indonesian"; } ?>"><?php if($data["national"] == "indonesian"){ echo "tourist"; }else{ echo "indonesian"; } ?></option>
                    </select>

                </div>
            </div>
            <br>
            <br>
            <div data-aos="zoom-in">
                <div class="kotak_password">
                    <label for="username" class="lab_username">Password :</label>
                    <input type="password" name="password" class="inp_password" id="input_pw" value="" placeholder=" jangan diisi,Jika tidak ingin diubah">
                    
                </div>
            </div>
            <div data-aos="zoom-in-down">
                <table  class="kotak_box">
                    <tr>
                    <td>
                        <div>
                            <input type="checkbox" name="lihat_pw" id="lihat_pw" class="kotak_pw" onclick="lihat_password()">
                        </div>
                    </td>
                    <td class="default-text-box"><label class="pw_text" for="lihat_pw"> Show Password</label></td>
                    </tr>
                    <tr>
                    <td>
                    <input type="hidden" value="" id="terima_text" name="text_terima">
                    </td>
                    </tr>
                </table>
            </div>
            <div>
                <div class="kotak_submit">
                <button type="submit" class="submit_log" id="button">UPDATE</button><br>
                </div>
                <div class="kotak_logout">
                <a href="logout.php?status=keluar" class="logout" >LOGOUT</a>
                </div>
            </div>
            
            
            <div data-aos="fade-down" data-aos-easing="linear" data-aos-duration="500">
               
                <br>
            </div>
        </form>
       </div>
       <script src="https://code.jquery.com/jquery-3.7.0.min.js" integrity="sha256-2Pmvv0kuTBOenSvLm6bvfBSSHrUJ+3A7x6P5Ebd07/g=" crossorigin="anonymous"></script>
    </div>
    </div>
    <script src="<?=BASEURL;?>/aos/dist/aos.js"></script>
    <script>
          <?php 
        if(isset($_SESSION['id_user']) == ""){
        ?>
        var login = document.getElementById("login");
        var profile = document.getElementById("profile");

        login.style.display = "block";
        profile.style.display = "none";
        <?php
        }else{
        ?>
        login.style.display = "none";
        profile.style.display = "block";
        <?php
        }
        ?>
        const terima = document.getElementById("terima_text");
      AOS.init({
        easing: 'ease-in-out-sine'
      });
      function lihat_password(){
        var pw = document.getElementById("input_pw");
        if(pw.type == "password"){
            pw.type = "text";
        }else{
            pw.type = "password"
        }
      }
      alertStatus = (e) => {
  if ($("#setuju").is(":checked")) {
    terima.value ="setuju";
  } else {
    terima.value ="tak_setuju";
  }
};

// Attaching the click event on the button
$(document).on("click", "#button", alertStatus);
    </script>
</body>
</html>